...
function LoginPage() {
    ...
    return (
        ...
        <div className="h-screen bg-slate-950 text-white flex justify-center items-center">
            <div className="w-10/12 sm:w-3/4 md:w-7/12 lg:w-2/5 xl:w-1/3 2xl:w-1/4 flex flex-col gap-4 p-6 items-center border-2 bg-slate-900 border-slate-800 rounded-md">
                ...
                <form className="w-full flex flex-col gap-4 text-sm md:text-base">
                    ...
                    <div className="flex flex-col gap-2">
                        <Link className="text-sky-600 hover:text-sky-800" to="/register">No account? Register now</Link>
                        <button className="bg-sky-600 hover:bg-sky-800 text-white p-2 rounded-lg" type="submit" onClick={handleSubmit} disabled={isLoggingIn}>
